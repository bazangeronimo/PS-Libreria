...
namespace PS.Template.Aplication.Services
{
    public class UserServices : IUserService
    {
        ...
        public Response GetUserById(int id, int owner)
        {
            ...
            var followed = _followQuery.searchFollowExist(owner, id);
            if (followed.objects == null)
                responseObject.followedOrNot = false;
            else
                responseObject.followedOrNot = true;
        ...
        public Response UpdateDescription(string user, dtoUpdateDescriptionUser description)
        {
            ...
            try
            {
                ...
                response = _userCommands.UpdateDescription(userFound, description.description);
                return response;
            }
            catch
            {
        ...
        public Response GetUsersRandom(string userr)
        {
            var response = new Response(true, "aleatorio");
            try
            {
                var seguidos = _followQuery.searchFollowsByUser(int.Parse(userr));
                var listaUsers = _userQuery.GetUserRandom(int.Parse(userr));
                List<object> noSeguidos = new();
                List<object> noSeguidosNoRepetidos = new();
                List<object> noRepetidos = new();
                ...
                foreach (var item in seguidos)
                {
                    ...
                    {
                        if (item.seguido_Fk != user.Usuario_Id && user.followers == null) 
                        {
                            noSeguidos.Add(user);
                            noSeguidosNoRepetidos = noSeguidos.Distinct().ToList();
                        }
                    }
                }

                if (noSeguidosNoRepetidos.Count == 0)
                {
                    if (listaUsers.Count > 5)
                    {
                        while (noRepetidos.Capacity != 5)
                        {
                            int aleatorio = random.Next(0, listaUsers.Count - 1);
                            var userPosicion = listaUsers.ToArray()[aleatorio];
                            users.Add(userPosicion);
                            noRepetidos = users.Distinct().ToList();
                        }
                    }
                    else
                    {
                        noRepetidos.Add(listaUsers);
                    }
                }

                if (noSeguidosNoRepetidos.Count <= 5)
                {
                    noRepetidos.Add(noSeguidosNoRepetidos);
                }
                else
                {
                    while (noRepetidos.Capacity != 5)
                    {
                        int aleatorio = random.Next(0, noSeguidosNoRepetidos.Count - 1);
                        var userPosicion = noSeguidosNoRepetidos.ToArray()[aleatorio];
                        users.Add(userPosicion);
                        noRepetidos = users.Distinct().ToList();
                    }
                }
                foreach (var user in noRepetidos)
                {
                    array.Add(user);
                }
                response.arrList = array;
