...
namespace FulvoDevs.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public IActionResult GetUserById(int id)
        {
            try
            {
                var owner = Convert.ToInt32(User.Identity.Name);
                var response = _userServices.GetUserById(id, owner);
        ...
        [HttpGet("userRandom"), Authorize]
        public IActionResult GetPhotoRandom()
        {
            try
            {
                var user = User.Identity.Name;
                var response = _userServices.GetUsersRandom(user);
                ArrayList array = new();
                if (!response.succes)
                {
                    return new JsonResult(new { error = response.content }) { StatusCode = response.StatusCode };
